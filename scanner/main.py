...
ALIASES: dict[str, str] = {
    ...
    "昇達科": "3491", "森達科": "3491",  # Whisper 常將「昇達科」誤轉為「森達科」
...
_SECTOR_CONTEXT_FAMILIES: dict[str, dict[str, set[str]]] = {
    ...
    "華新麗華集團": {
        ...
        "keywords": {
            "華新", "華興", "華新麗華", "華興立華", "華興集團", "華新集團",
            "被動元件", "被動員間", "新唐", "華邦", "華邦店", "彩金", "新商店",
        },
    },
}
...
CONTEXT_REQUIRED: dict[str, list[str]] = {
    ...
    "4716": ["4716", "大立醫", "醫療器材", "醫材", "手術"],      # 「大立」是大立光(3008)子字串，Whisper變體無限(光/官/廣…) → 需醫療詞確認
    "1459": ["1459", "聯發紡", "紡織", "棉紗", "聯發股"],        # 「聯發」幾乎等於聯發科(2454)，1459紡織股極少被財經節目提及
    "1210": ["1210", "大成食品", "雞肉", "飼料", "食品股"],      # 「大成」出現在「集大成」等成語及一般語境，需食品/飼料詞確認
    "3162": ["3162", "精確科技", "精確儀器"],                    # 「精確」是常見形容詞（做到很精確、精確度高），需股號或公司全名
    "2371": ["2371", "大同電", "大同公司", "大同股", "大同冰箱", "大同電鍋"],  # 「大同」極常見（大同小異、世界大同），需公司專屬詞確認
    "8112": ["8112", "至上電機", "至上電"],    # 「至上」常出現在「X至上主義」「甚至上游」等一般語境
    "5410": ["5410", "國眾電腦", "國眾電"],    # 「國眾」常出現在「各國眾多」「美國眾議院」等一般語境
    "6654": ["6654", "天正國際", "天正電機"],  # 「天正」常出現在「那一天正式上路」等一般語境
    "1464": ["1464", "得力工業", "紡織"],      # 「得力」常出現在「獲得力量」「得力助手」等一般語境
}
...
KEYWORD_PATTERN_OVERRIDE: dict[str, str] = {
    ...
    "天宇": r"(?<![昨今明前後每])天宇",  # exclude 「昨天宇翔哥」等時間詞+人名跨字— 8171 天宇
    "大洋": r"大洋(?!洲)",              # exclude 「大洋洲」(Oceania) — 1321 大洋電線電纜
    # 大立(4716)/聯發(1459)/大成(1210) 改用 CONTEXT_REQUIRED，不再用 lookahead（Whisper 變體無限）
}
...
_COMMON_STOCK_NAME_SUFFIXES = (
    "半導體", "電子", "科技", "光電", "工業", "材料", "生技", "網路", "國際", "資訊",
)
